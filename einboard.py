...
def calculate_kpis():
    ...
    if not df.empty:
        for scope in ["Scope 1","Scope 2","Scope 3"]: summary[scope]=df[df["Scope"]==scope]["Quantity"].sum()
...
def render_ghg_dashboard(include_data=True):
    ...
    if include_data:
        ...
        if scope!="Scope 3": sub_activity = st.selectbox("Select sub-activity", list(sub_options.keys())); st.info(sub_options[sub_activity])
        else: sub_activity = st.selectbox("Select sub-category", list(sub_options.keys()))
        specific_item = None
        if scope=="Scope 3": items = scope_activities[scope][activity][sub_activity]; specific_item = st.selectbox("Select specific item", items) if items else None
        unit = units_dict.get(sub_activity,"Number of flights" if sub_activity=="Air Travel" else "km / kg / tonnes")
        quantity = st.number_input(f"Enter quantity ({unit})", min_value=0.0, format="%.2f")
        st.file_uploader("Upload CSV/XLS/XLSX/PDF for verification (optional)", type=["csv","xls","xlsx","pdf"])
...
def render_energy_dashboard(include_entry=False):
    ...
    if not df.empty:
        ...
        trend = df.groupby(["Month","Type"])["Energy_kWh"].sum().reset_index()
