...
PROFESSIONAL_COLORS = {"Scope 1": "#1f77b4", "Scope 2": "#ff7f0e", "Scope 3": "#2ca02c"}
...
with c2:
    st.markdown(f"<div class='kpi' style='color:{PROFESSIONAL_COLORS['Scope 1']}'><div class='kpi-value'>{format_indian(s1)}</div><div class='kpi-label'>Scope 1 (tCO₂e)</div></div>", unsafe_allow_html=True)
with c3:
    st.markdown(f"<div class='kpi' style='color:{PROFESSIONAL_COLORS['Scope 2']}'><div class='kpi-value'>{format_indian(s2)}</div><div class='kpi-label'>Scope 2 (tCO₂e)</div></div>", unsafe_allow_html=True)
with c4:
    st.markdown(f"<div class='kpi' style='color:{PROFESSIONAL_COLORS['Scope 3']}'><div class='kpi-value'>{format_indian(s3)}</div><div class='kpi-label'>Scope 3 (tCO₂e)</div></div>", unsafe_allow_html=True)
...
if not df_log.empty:
    ...
    if not df_cycle.empty:
        ...
        fig_pie = px.pie(pie_df, names="Scope", values="Emissions (tCO₂e)", hole=0.45,
                         color="Scope", color_discrete_map=PROFESSIONAL_COLORS, template="plotly_dark")
        ...
        for scope in ["Scope 1", "Scope 2", "Scope 3"]:
            ...
            fig_line.add_scatter(x=MONTH_ORDER, y=y, mode="lines+markers", name=f"{scope} Actual", line=dict(color=PROFESSIONAL_COLORS[scope]))
            fig_line.add_scatter(x=MONTH_ORDER, y=forecast_vals, mode="lines+markers", name=f"{scope} Forecast", line=dict(color=PROFESSIONAL_COLORS[scope], dash="dash"))
