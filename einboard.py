...
# Session state
# ---------------------------
if "entries" not in st.session_state: st.session_state.entries = pd.DataFrame(columns=["Scope","Activity","Sub-Activity","Specific Item","Quantity","Unit"])
if "page" not in st.session_state: st.session_state.page = "Home"
if "renewable_list" not in st.session_state: st.session_state.renewable_list = []
...
def render_energy_dashboard(include_entry=True):
    ...
    for col,label in zip(cols,["Total","Fossil","Renewable"]):
        ...
        <div class='kpi'>
            ...
            <div class='kpi-label'>{label.lower()} energy</div>
        </div>
        """,unsafe_allow_html=True)
    ...
    if not all_energy.empty:
        ...
        fig = px.bar(monthly_trend, x="Month", y="Energy_kWh", color="Fuel_Source",
                     title="Monthly Energy Consumption Trend", text_auto=".2s")
        fig.update_layout(barmode="stack", plot_bgcolor="#0d1117", paper_bgcolor="#0d1117",
                          font_color="#e6edf3", xaxis_title="Month", yaxis_title="Energy (kWh)")
        st.plotly_chart(fig, use_container_width=True)

    # Renewable energy entry
    if include_entry:
        st.subheader("Add Renewable Energy Entry")
        fuel_source = st.selectbox("Source", ["Solar","Wind","Biogas","Purchased Green Energy"])
        month = st.selectbox("Month", months)
        energy_kwh = st.number_input("Energy (kWh)", min_value=0.0, format="%.2f")
        co2e = st.number_input("CO2e (kg)", min_value=0.0, format="%.2f")
        if st.button("Add Renewable Entry"):
            st.session_state.renewable_list.append({"Fuel_Source":fuel_source,"Month":month,"Energy_kWh":energy_kwh,"CO2e_kg":co2e})
