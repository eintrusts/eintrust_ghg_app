...
if "energy_data" not in st.session_state:
    st.session_state.energy_data = pd.DataFrame(columns=["Location","Month","Energy_kWh","CO2e_kg","Type","Fuel","Source"])
...
def render_energy_dashboard(include_entry=True):
    ...
    if not st.session_state.energy_data.empty:
        total_energy = st.session_state.energy_data.groupby("Type")["Energy_kWh"].sum().to_dict()
        total_co2e = st.session_state.energy_data.groupby("Type")["CO2e_kg"].sum().to_dict()
    else:
        total_energy = {"Fossil":0,"Renewable":0}
        total_co2e = {"Fossil":0,"Renewable":0}
    ...
    # Trend stacked bar
    if not st.session_state.energy_data.empty:
        trend = st.session_state.energy_data.groupby(["Month","Type"]).sum().reset_index()
    ...
    if include_entry:
        ...
        for i in range(int(num_entries)):
            col_loc,col_src,col_val = st.columns([2,2,2])
            ...
            for m in months:
                renewable_list.append({"Source":source,"Location":location,"Month":m,"Energy_kWh":monthly_energy,
                                       "Type":"Renewable","CO2e_kg":monthly_energy*emission_factors_energy.get(source,0)})
        if renewable_list:
            renewable_df = pd.DataFrame(renewable_list)
            st.session_state.energy_data = pd.concat([st.session_state.energy_data, renewable_df], ignore_index=True)
