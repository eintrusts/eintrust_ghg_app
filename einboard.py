...
authenticator = stauth.Authenticate(credentials, "einboard_cookie", "secret_key", cookie_expiry_days=30)
...
if authentication_status == False:
    ...
    # --- SharePoint Config ---
    sharepoint_url = "https://yourcompany.sharepoint.com/sites/EinTrust"  # 🔄 replace with your site
    username_sp = "your_email@company.com"   # 🔄 replace
    password_sp = "your_password"            # 🔄 replace

    ctx = ClientContext(sharepoint_url).with_credentials(UserCredential(username_sp, password_sp))

    def save_to_sharepoint(df, client_name):
        file_name = f"{client_name}_emissions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        buffer = io.BytesIO()
        df.to_excel(buffer, index=False)
        buffer.seek(0)
        target_folder = ctx.web.get_folder_by_server_relative_url("Shared Documents/EinBoardData")
        target_folder.upload_file(file_name, buffer.read())
        ctx.execute_query()
        ...
        if uploaded_file:
            try:
                ...
                if not required_cols.issubset(df.columns):
                    ...
                    save_to_sharepoint(log_df, username)

                    st.success("CSV processed and saved to SharePoint successfully!")
            except Exception as e:
                st.error(f"Failed to process CSV: {e}")

    # --- Dashboard ---
    else:
        # 🚫 Removed Emission Factors Table display
        ...
        if add_mode:
            ...
            if st.sidebar.button("Add Entry") and quantity>0 and ef>0:
                ...
                log_df = pd.DataFrame(st.session_state.emissions_log)
                save_to_sharepoint(log_df, username)
                st.success("Entry saved to SharePoint ✅")
        ...
        if st.session_state.emissions_log:
            ...
            if st.button("Delete Selected Rows") and selected_rows:
                ...
                # --- Save to SharePoint ---
                save_to_sharepoint(pd.DataFrame(st.session_state.emissions_log), username)
